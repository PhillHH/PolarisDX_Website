...
// Typ für technische Spezifikationen (Schlüssel-Wert-Paare)
type TechSpec = {
  parameter: string
...
/**
 * ProductPage Komponente.
 * Zeigt die Detailansicht eines einzelnen Produkts.
 */
const ProductPage = () => {
  const { t } = useTranslation(['shop', 'common'])
  const { slug } = useParams<{ slug: string }>()

  // Produktsuche anhand des Slugs
  ...
  // Hilfsfunktion, um Arrays aus den Übersetzungsdateien sicher abzurufen
  const getList = (key: string): string[] => {
    const result = t(key, { returnObjects: true })
    return Array.isArray(result) ? result as string[] : []
  }

  // Hilfsfunktion für TechSpecs (Array von Objekten)
  const getTechSpecs = (key: string): TechSpec[] => {
    const result = t(key, { returnObjects: true })
    return Array.isArray(result) ? result as TechSpec[] : []
  }

  // Abruf der lokalisierten Produktdetails
  ...
  return (
    <div className="bg-slate-50 text-gray-900">
      ...
      <main className="mx-auto max-w-container px-4 py-12 lg:grid lg:grid-cols-[minmax(0,3fr)_minmax(0,1.5fr)] lg:items-start lg:gap-12 lg:px-0 lg:py-16">
        ...
        {/* Sidebar: Bestellinfo & Ähnliche Produkte */}
