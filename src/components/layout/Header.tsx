                          ...
                          <div className="grid grid-cols-2 gap-x-2 divide-x divide-slate-200/70 px-2">
                            {item.groups.map((group) => (
                              <div key={group.heading} className="px-4 py-2">
                                <p className="mb-2 px-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-400">
                                  {t(`nav.${group.heading}`)}
                                </p>
                  ...
                  {item.children || item.groups ? (
                    <div>
                      {/* Ein button, kein div mit onClick: der Aufklapper war
                          ...
                          das gesamte Untermenue war damit fuer Tastaturnutzer
                          unerreichbar. */}
                      <button
                        type="button"
                        className={`flex min-h-[44px] w-full items-center justify-between text-left text-lg font-light tracking-wide text-white`}
                        onClick={() =>
                      ...
                      {openSubmenu === item.label && (
                        <div className={`pl-4 mt-3 space-y-3 border-l-2 border-white/20`}>
                          ...
                          {/* Gruppierte Fassung: Ueberschrift je Block, damit die
                              Trennung POC / Labor auch mobil sichtbar bleibt. */}
                          {item.groups?.map((group) => (
