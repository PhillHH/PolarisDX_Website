...
const RoiCalculatorSection = () => {
  ...
  const eur = (value: number) => formatCurrency(value, locale)
    ...
    <section id="roi-rechner" className="bg-slate-50">
      <div className="mx-auto max-w-container px-4 py-24 lg:py-24 lg:px-0">
        ...
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-base leading-relaxed text-gray-600">{t('roi.intro')}</p>
        </div>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2">
          ...
          <div className="rounded-2xl bg-brand-deep p-7 text-white">
            {/* Grosse Kennzahl */}
            <p className="text-sm text-white/70">{t('roi.out.month')}</p>
            ...
            {isNegative ? (
              ...
              <dl className="mt-6 space-y-3 text-sm">
                <div className="flex items-center justify-between gap-4 border-t border-white/10 pt-3">
                  <dt className="text-white/70">{t('roi.out.revenue')}</dt>
                  <dd className="font-medium">{eur(revenuePerMonth)}</dd>
                </div>
                <div className="flex items-center justify-between gap-4 border-t border-white/10 pt-3">
                  <dt className="text-white/70">{t('roi.out.year')}</dt>
                  <dd className="font-medium">{eur(dbPerYear)}</dd>
                </div>
                <div className="flex items-center justify-between gap-4 border-t border-white/10 pt-3">
                  <dt className="text-white/70">{t('roi.out.per_test')}</dt>
                  <dd className="font-medium">{eur(dbPerTest)}</dd>
                </div>
                {payback != null ? (
                  <div className="flex items-center justify-between gap-4 border-t border-white/10 pt-3">
                    <dt className="text-white/70">{t('roi.out.payback')}</dt>
          ...
          <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-7">
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid gap-6 sm:grid-cols-2">
                  ...
                  <div className="flex flex-col">
                    ...
                    >
                      ...
                      <option value="longevity">{t('roi.form.opt_longevity')}</option>
                      <option value="other">{t('roi.form.opt_other')}</option>
                    </select>
                  </div>
                ...
                >
                  {status === 'submitting' ? t('roi.form.sending') : t('roi.form.submit')}
                </Button>
              </form>
            )}
          </div>
