...
| **AP24** | `src/components/**`, `consumer/**`, `befund/**`, `tailwind.config.js`, `ci.yml`, `eslint.config.js`                                                        | BD: AP05 (Kontrast)                        | SAFE*PARALLEL *(zu AP25/AP26)\_                          | **Gate 11.** Skip-Link existiert site-weit nicht.                                      |
| **AP25** | `vite.config.ts`, `entry-client.tsx`, Assets, `server.ts`, `ci.yml`, `pages/musterbefund/**`                                                               | BD: AP16 (Chunk-Split)                     | SAFE*PARALLEL *(zu AP24/AP26)\_                          | Lighthouse-CI-Schwellen in `ci.yml`.                                                   |
| **AP26** | `server.ts` (HSTS/CSP), `server/server.js` (Limits), Docker-Dateien, `ci.yml`                                                                              | BD: AP23/AP06 (Chat, S10), AP22, AP28      | SAFE*PARALLEL *(zu AP24/AP25)\_                          | **Gate 12.** `X-Powered-By` bereits erledigt; HSTS offen.                              |
...
| **AP29** | keine (ggf. `LEGACY_PATH_REDIRECTS`, `robots.txt`)                                                                                                         | BD: AP09, AP10, AP21                       | SAFE*PARALLEL *(Analyse)\_                               | Fast kein Code-Fußabdruck. S13 vor AP31.                                               |
| **AP30** | keine (Verifikation)                                                                                                                                       | BD: **AP27** (S12)                         | SAFE*PARALLEL *(Vorbereitung)\_                          | Kein Code-Fußabdruck. PT30.5.1 friert den SHA.                                         |
| **AP31** | keine (Betrieb)                                                                                                                                            | BD: AP27, AP29, AP30 (S12–S14)             | **SERIAL_RECOMMENDED**                                   | Kein Code-Fußabdruck. Cutover + Rollback.                                              |
| **AP32** | keine (Beobachtung)                                                                                                                                        | BD: AP31 (S15)                             | SAFE*PARALLEL *(nach Launch)\_                           | Kein Code-Fußabdruck. Erzeugt Folge-Tickets.                                           |
