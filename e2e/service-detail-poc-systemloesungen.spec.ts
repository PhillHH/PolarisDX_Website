...
test.describe('PT13.5 POC system solutions service detail', () => {
  for (const locale of SUPPORTED_LANGUAGES) {
    test(`${locale} renders normalized POC system content and SEO`, async ({ page }) => {
      const response = await page.goto(`/${locale}/diagnostics/poc-systemloesungen`)
      ...
      await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
        'href',
        `https://polarisdx.net/${locale}/diagnostics/poc-systemloesungen`,
      )
      await expect(page.locator('link[rel="alternate"][hreflang]')).toHaveCount(11)
      await expect(page.locator('link[rel="alternate"][hreflang="x-default"]')).toHaveAttribute(
        'href',
        'https://polarisdx.net/de/diagnostics/poc-systemloesungen',
      )
  ...
  test('uses real articles and IglooPro target plus deterministic GENERAL_SALES', async ({
    page,
  }) => {
    await page.goto('/de/diagnostics/poc-systemloesungen')
    await expect(page.getByRole('link', { name: /IglooPro im Systemkontext/ })).toHaveAttribute(
      'href',
      '/de/igloo-pro',
    )
  ...
  test('visible FAQ and FAQ schema are identical and avoid unsupported system promises', async ({
    page,
  }) => {
    await page.goto('/de/diagnostics/poc-systemloesungen')
    ...
    expect(normalizedContent).not.toMatch(
      /garantiert|schlüsselfertig|nahtlose Integration|3.?5 Werktage|Termin buchen|Demo buchen|SLA/i,
    )
    expect(normalizedContent).toContain('keine automatische Integration')
  })
  ...
  for (const viewport of [
    ...
    test(`${viewport.label} has no horizontal overflow and keeps focus visible`, async ({
      ...
      await page.goto('/pl/diagnostics/poc-systemloesungen')
      expect(
