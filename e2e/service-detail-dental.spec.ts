...
test.describe('PT13.2 Dental service detail', () => {
  for (const locale of SUPPORTED_LANGUAGES) {
    test(`${locale} renders normalized Dental content and SEO`, async ({ page }) => {
      const response = await page.goto(`/${locale}/diagnostics/dental`)
      ...
      await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
        'href',
        `https://polarisdx.net/${locale}/diagnostics/dental`,
      )
      await expect(page.locator('link[rel="alternate"][hreflang]')).toHaveCount(11)
      await expect(page.locator('link[rel="alternate"][hreflang="x-default"]')).toHaveAttribute(
        'href',
        'https://polarisdx.net/de/diagnostics/dental',
      )
  ...
  test('uses real Registry crosslinks, related content and GENERAL_SALES attribution', async ({
    page,
  }) => {
    await page.goto('/de/diagnostics/dental')
    await expect(page.getByRole('link', { name: /Vitamin D und Implantologie/ })).toHaveAttribute(
      'href',
    ...
    for (let index = 0; index < 2; index += 1) {
      ...
      await expect(salesActions.nth(index)).toHaveAttribute('data-service-family', 'dental')
    }
  })
  ...
  test('visible FAQ and FAQ schema are identical and claim-safe', async ({ page }) => {
    ...
    expect(normalizedContent).not.toMatch(
      /Umsatz|Marge|Amortisation|Erfolgsgarantie|Implantatgarantie/i,
    )
  })
  ...
  for (const viewport of [
    ...
    test(`${viewport.label} has no horizontal overflow and keeps focus visible`, async ({
      ...
      await page.goto('/pl/diagnostics/dental')
      expect(
