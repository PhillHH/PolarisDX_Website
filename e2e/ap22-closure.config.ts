 ...
 *
 * Gemessen wird, was ein Unit-Test nicht kann: welche Fremd-Origins der
 * Browser VOR einer Einwilligung wirklich kontaktiert, und ob eine
 * abgelehnte Analytics-Einwilligung den Geschaeftsvorgang blockiert.
 *
 * `dist/` bleibt unberuehrt — gebaut wird in `node_modules/.cache/pt21-7`.
 */
const port = Number(process.env.E2E_PORT ?? 3482)
const backendPort = Number(process.env.E2E_BACKEND_PORT ?? 5482)
const cryptoCompat = fileURLToPath(new URL('./node18-crypto-hash.cjs', import.meta.url))
const clientOutDir = 'node_modules/.cache/ap22-closure/client'
const serverOutDir = 'node_modules/.cache/ap22-closure/server'
...
export const LEAD_DB_PATH =
  process.env.AP22C_LEAD_DB ?? 'node_modules/.cache/ap22-closure/leads.sqlite3'
...
export default defineConfig({
  testDir: '.',
  testMatch: ['ap22-closure.spec.ts'],
  timeout: 120_000,
  fullyParallel: false,
  retries: 0,
  workers: 1,
