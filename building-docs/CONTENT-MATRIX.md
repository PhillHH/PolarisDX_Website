...
| ---------- | ----------- | -------------------------------------------------- | --------- | ---------------------------------- | -------------------- | ------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------- | ------ |
| **C-01**   | P-01        | `/`                                                | T1        | Hero · Proof · Pillars · FAQ · CTA | `PARTIAL`            | `READY`       | `public/locales/*/home.json` (261 Leaf-Werte), 9 Home-Sections                             | 10/10; PT11.1–PT11.4 produktiv, Rest PT11.5–PT11.6  | 10     |
...
| **C-22**   | P-01        | Homepage Trust/Proof (4 Signale + 1 Referenz)      | T1        | Proof                              | `READY`              | `READY`       | `TrustBar.tsx` + `src/data/testimonials.ts` + `*/home.json` `trustbar`/`testimonials.*`    | 10/10; Claim Register `HCL-002`–`HCL-006`           | 10     |
...
| **C-22**   | `NONE`                                                                                    | 1 produktiv genutztes reales Avatar; 3 weitere vorhanden, `martin_fischer` ohne Avatar | teils     | nein             | nein               | nein           | AP04 · AP11      | PT11.2 zeigt nur `bastian_wessing` mit claim-reduziertem `proof_text`; Rating/Carousel/Partnerlogo 0; vier weitere Datensätze bewusst nicht im Homepage-Output                                                                          |
...
Alle fünf Quelldatensätze tragen `text: ''` in `src/data/testimonials.ts`; der vollständige Text liegt
korrekt lokalisiert in `home.json` unter `testimonials.<id>.text` (× 10, übersetzt). PT11.2 rendert
ausschließlich die bereits produktive namentliche Referenz `bastian_wessing` und dafür das
claim-reduzierte `proof_text` × 10; die übrigen vier Datensätze sind mangels gleich starker
Freigabeevidenz nicht Teil des produktiven Homepage-Outputs. Das leere Feld ist ein **totes Feld** wie
