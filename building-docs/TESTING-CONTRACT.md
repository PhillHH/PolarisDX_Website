...
| PT276-F9  | Die Alt-Baselines in `e2e/design-system.spec.ts-snapshots` stammen aus `6596a90` (2026-08-28, = `origin/main`); die Relaunch-Arbeit hat danach `src/index.css`, `tailwind.config.js`, Header und Abschnitte geaendert. 20 der 35 Vergleiche scheitern — nicht an Schriftrendering, sondern strukturell (z. B. erwartet 1440x19222, erhalten 1440x4129; erwartet 390x32101, erhalten 390x7564). In CI (Lauf 35074608105, Job `quality`) und lokal im sauberen Checkout identisch 20 failed / 15 passed. **Kein Neu-Baselining in PT27.6**: der Delta ist ein dokumentationspflichtiger Design-Vorgang (DESIGN-SYSTEM-CHANGELOG), kein Testproblem | LAUNCH_BLOCKER (Design)                 | AP05/AP06 / Design-Owner   |
| PT276-F10 | AP26 `e2e/pt26.2.spec.ts` ist nicht stabil: in CI faellt „Zustimmung … alle Ziele stehen in der Allowlist" (kein `/g/collect` in 15 s), lokal zweimal reproduzierbar ein **anderer** Test, „Laufzeit vor der Einwilligung" (`form [role="status"]` erscheint nicht). Der Job `security-runtime` lief am 2026-09-16 erstmals ueberhaupt auf GitHub. AP26-Eigentum; nicht abgeschwaecht, keine Schwelle veraendert                                                                                                                                                                                                                                 | LAUNCH_BLOCKER (AP26)                   | AP26 / AP27-CLOSURE        |
| PT276-F11 | `pt24.2` „Pfeil ab oeffnet und fuehrt durch die Liste, Home und End springen" scheiterte in CI einmalig (`Home springt nicht an den Anfang`); lokal 41/41 in zwei Vollstaendigkeitslaeufen und 280/280 der Gesamtsuite. Die HMR-WebSocket-Fehler (`ENOTFOUND relaunch.polarisdx.net`, aus `vite.config.ts`) treten lokal ebenso auf und scheiden als Unterschied aus. Im Wiederholungslauf desselben CI-Jobs 41/41 gruen — Flake bestaetigt, Ursache weiter offen. Nicht per Retry maskiert                                                                                                                                                      | FLAKE (offen)                           | AP24 / AP27-CLOSURE        |
| PT276-F12 | `e2e/pt24.6.spec.ts` loeste `axe-core` ueber einen absoluten Pfad der Entwicklungsmaschine auf (`createRequire('/home/phillip/01polaris-preview/package.json')`). Lokal fiel das nie auf, im CI-Runner scheiterte der axe-Gate mit `Cannot find module 'axe-core/axe.min.js'` (Lauf 35074608105, Wiederholung des Jobs). **In PT27.6 behoben** (`createRequire(import.meta.url)`), ohne Regel- oder Schwellenaenderung. Dieselbe Klasse steckt noch in `e2e/ap20-pt201/202/203.config.ts` (`/home/phillip/.cache/...`); diese Configs laufen in keinem CI-Job und bleiben unangetastet                                                           | CI-Portabilitaet (behoben) / Restschuld | AP27-CLOSURE               |
...
| 2026-09-16 | PT27.6 | CI-Evidence: Snapshot-Commit auf `ci/ap27-evidence-2026-09-16` (`e14e904`), Lauf 35074608105 — 9 Jobs gruen (inkl. `ap27-visual`, `ap27-guards`, `ap27-journeys`, AP25, AP26 `security`), 3 rot; Befunde PT276-F9..F11; CI-CONTRACT §1 auf gemessene GitHub-Wahrheit |
